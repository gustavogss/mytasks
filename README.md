# Projeto mytasks
- Aplicativo de listas de tarefas para Android e IOS
## Tecnologias e Ferramentas utilizadas :robot:
- [Expo](https://docs.expo.dev/)
- [React Native v0.71.8](https://reactnative.dev/docs/environment-setup)
- [Typescript](https://docs.expo.dev/guides/typescript/)
- [StyleSheet](https://snack.expo.dev/@aboutreact/stylesheet-example?session_id=snack-session-45GVESXWi)
- [Vector Icons](https://docs.expo.dev/guides/icons/)
- [Flat List](https://reactnative.dev/docs/flatlist)

## Aplicativo :iphone: